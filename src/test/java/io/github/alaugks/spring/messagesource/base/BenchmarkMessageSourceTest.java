...
/**
 * This test compares the logic when resolving the code of BaseMessageSourceBuilder vs. ResourceBundleMessageSource
 * and ReloadableResourceBundleMessageSource. Behaviour must be equal.
 */
@SuppressWarnings({"java:S125"})
class BenchmarkMessageSourceTest {

	static BaseMessageSourceBuilder baseMessageSourceBuilder;

	static BaseMessageSourceBuilder baseMessageSourceBuilderICU4j;
	...
	static void beforeAll() throws IOException {
		baseMessageSourceBuilder = BaseMessageSourceBuilder
				.builder(defaultLocale, createTransUnitListFromMessagesPropertiesFiles())
				.build();

		baseMessageSourceBuilderICU4j = BaseMessageSourceBuilder
				.builder(defaultLocale, createTransUnitListFromMessagesPropertiesFiles())
				.enableICU4j()
				.build();
	...
	void test_base_message_source(String locale, String code, Object[] args, Object expected) {
		assertEquals(expected, baseMessageSourceBuilder.getMessage(
				code,
				args,
	...
	void test_base_message_source_icu4j(String locale, String code, Object[] args, Object expected) {
		assertEquals(expected, baseMessageSourceBuilderICU4j.getMessage(
			code,
			args,
			Locale.forLanguageTag(locale)
		));
	...
	private static List<TransUnitInterface> createTransUnitListFromMessagesPropertiesFiles() throws IOException {
		...
		for (TransFileInterface translationFile : resourcesLoader.getTranslationFiles()) {
